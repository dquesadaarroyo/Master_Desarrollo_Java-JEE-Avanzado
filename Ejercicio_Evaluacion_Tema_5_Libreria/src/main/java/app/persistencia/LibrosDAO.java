...
public class LibrosDAO implements ItfzLibrosDAO{
	...
	public List<Libro> autoCompletaTitulo(String titulo) {
		try{
			session = getHibernateTemplateUpdate();
			Query q = session.createQuery("select l.id from Libro l where l.titulo like :titulo");
			q.setParameter("titulo", "%" + titulo +"%");
			misLibros = q.list();
	...
	public List<Libro> librosPorTitulo(String titulo) throws LibroNoEncontradoException{
		try{	
			session = getHibernateTemplateUpdate();
			Query q = session.createQuery("select l from Libro l where l.titulo = :titulo");
			q.setParameter("titulo", titulo);
			misLibros = q.list();
